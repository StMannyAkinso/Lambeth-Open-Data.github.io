...
async function displayChartsForSector(sectorId, sectorsMap, chartsMap) {
  ...
  const sectorDescription = sectorsMap[sectorId].description;

  mainSection.innerHTML = `<div class='sector-details'><h1 style='color:${sectorColor};'>${sectorName} data</h1><p style='color:${sectorColor};'>${sectorDescription}</p></div>`;
